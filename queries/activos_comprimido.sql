...
CREATE TABLE activos_comprimido (
  `id` int(11) NOT NULL,
...
insert into activos_comprimido select * from activos where year(fecha_ingreso) > 0 and year(fecha_alta) > 0;

SELECT * FROM issste.activos WHERE YEAR(fecha_ingreso) = 1972;
SELECT * FROM issste.activos_comprimido WHERE YEAR(fecha_ingreso) = 1972;